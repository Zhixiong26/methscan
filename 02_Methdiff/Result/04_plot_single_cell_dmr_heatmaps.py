...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--value-transform",
        choices=("mean-cpg-ratio", "dmr-zscore"),
        default="mean-cpg-ratio",
        help=(
            "Values displayed in the heatmap: raw mean CpG ratio, or Z-score "
    ...
    parser.add_argument(
        ...
        default=30,
        help=(
            "For dmr-zscore, minimum non-NA cells required for one DMR "
            "(default: 30)."
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=3.0,
        help="For dmr-zscore, clip displayed values to [-clip, +clip] (default: 3).",
    )
    parser.add_argument(
...
def dmrwise_zscore(
    matrix: np.ndarray, min_observed_cells: int, clip: float
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Standardize every DMR column across its observed (non-NA) cells."""
    if matrix.ndim != 2:
        raise ValueError("DMR Z-score requires a two-dimensional cell x DMR matrix")
...
def second_pass_build_display_matrix(
    ...
    if exact_dmr_columns:
        exact_matrix = np.full(
            (expected_cells, total_bins), np.nan, dtype=np.float32
        )
        sums = None
        counts = None
    else:
        exact_matrix = None
        sums = np.zeros((total_bins, expected_cells), dtype=np.float32)
        counts = np.zeros((total_bins, expected_cells), dtype=count_dtype)
    ...
    with gzip.open(matrix_path, "rt", encoding="utf-8", newline="") as handle:
        ...
        for line_number, line in enumerate(handle, start=2):
            ...
            if exact_dmr_columns:
                ...
                if valid.any():
                    sums[bin_index, valid] += values[valid]
    ...
    if exact_dmr_columns:
        ...
        del sums, counts
        display_matrix = display[:, cell_order].T
    return (
        display_matrix,
        bin_offsets,
        bin_first_row,
...
def process_sample(
    ...
    if value_transform == "dmr-zscore":
        ...
        matrix, zscore_observed_counts, zscore_eligible = dmrwise_zscore(
            matrix, zscore_min_observed_cells, zscore_clip
        )
        print(
            f"[{sample}] DMR-wise Z-score: "
            f"{int(zscore_eligible.sum()):,}/{expected_dmrs:,} DMRs eligible "
    ...
    if save_plot_matrix:
        matrix_field = (
            "dmrwise_zscore" if value_transform == "dmr-zscore" else "mean_cpg_ratio"
        )
        np.savez_compressed(
    ...
    print(
        f"[{sample}] complete: {expected_cells:,} cells x {expected_dmrs:,} all DMRs; "
...
def run(args: argparse.Namespace) -> Path:
    ...
    if args.value_transform == "dmr-zscore" and not args.exact_dmr_columns:
        raise ValueError("--value-transform dmr-zscore requires --exact-dmr-columns")
